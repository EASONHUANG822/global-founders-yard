...
export const metadata = {
  title: "Cross-border Business Teams | Global Founders Yard",
  description:
    ...
    <main className="min-h-screen bg-yard-black px-5 py-8 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="py-20 lg:py-28">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-white/[0.42]">Founder Fit &mdash; Cross-border</p>
          <h1 className="mt-6 max-w-5xl text-balance text-6xl font-black leading-[0.88] tracking-[-0.075em] sm:text-7xl lg:text-8xl">Design anywhere, build in Shenzhen.</h1>
          <p className="mt-8 max-w-3xl text-lg leading-8 text-white/[0.62]">
            Cross-border teams face unique challenges: coordinating across time zones, navigating different business cultures, and managing supply chains that span countries. Global Founders Yard provides the local presence and network that makes cross-border hardware companies work.
          </p>
        </section>

        <section className="grid gap-4 md:grid-cols-3">
          {[
            { title: "Supply chain coordination", description: "Manage suppliers, manufacturers, and logistics across borders with local presence and on-the-ground support in Shenzhen." },
            { title: "Market entry", description: "Navigate the Chinese market with guidance on regulatory requirements, business development, and local partnerships." },
            { title: "Global distribution", description: "Connect with logistics partners, trade facilitators, and distribution channels that bridge Shenzhen manufacturing with global markets." },
          ].map((card) => (
